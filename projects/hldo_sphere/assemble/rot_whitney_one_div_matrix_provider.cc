...
Eigen::MatrixXd RotWhitneyOneDivMatrixProvider::Eval(
  ...
  const double s0 = lf::mesh::to_sign(edgeOrientations[0]);
  const double s1 = lf::mesh::to_sign(edgeOrientations[1]);
  const double s2 = lf::mesh::to_sign(edgeOrientations[2]);
