 ...
 * @brief Solve hodge laplacian source problem for fixed function u = 5.2 and
 * fixed k = 100
 */
...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    std::cerr << "Usage: " << argv[0] << " max_refinement_level radius"
              << std::endl;
    exit(1);
  }
  ...
  for (lf::base::size_type lvl = 0; lvl < refinement_level + 1; ++lvl) {
    ...
    std::cout << "\nVector for two \n"
              << sol.mu_two << "\n\nDivergence of two\n"
              << std::get<0>(lse_builder.GetMuTwo()) << "\n\n"
              << "\n\nMatrix of two\n"
              << lse_builder.GetGalerkinMatrix(2).makeDense() << "\n\n"
              << "\n\nPhi of two\n"
              << lse_builder.GetLoadVector(2) << "\n\n";
  }
  ...
  for (lf::base::size_type lvl = 0; lvl <= refinement_level; ++lvl) {
    ...
    lf::io::VtkWriter writer(sol.mesh, concat("result_constant_", lvl, ".vtk"));

    // write the points out to the result vtk
    // first get dataset
    lf::mesh::utils::CodimMeshDataSet<double> data_set_solution_zero =
        projects::hldo_sphere::post_processing::extractSolution(sol.mesh,
                                                                sol.mu_two);
    writer.WriteCellData(concat("mf_two_", lvl), data_set_solution_zero);

    // writer.WriteCellData(concat("mf_diff_one_", lvl), mf_diff_one);
    // writer.WriteCellData(concat("mf_diff_two_", lvl), mf_diff_two);
