...
int main(int /*argc*/, char ** /*argv*/){
  ...
  Eigen::VectorXd numSteps(6); numSteps.setZero();
  Eigen::VectorXd tau(6); tau.setZero();
  double T = 1.;
  ...
  for(int l = 0; l < 6; l++) {
	...
	if(l < 5){
 	  std::cout << "errorL2 solcol0 for l = " << l << " : " << eL2(l) << std::endl;
	}
	
	std::cout << "number of timesteps for l = " << l << " : " << numSteps(l) << std::endl;
