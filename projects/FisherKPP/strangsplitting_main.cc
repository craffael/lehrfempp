...
int main(int /*argc*/, char** /*argv*/) {
   ...
   *
   * APPROACH 2: Take topography into account: Mountain chains impede the
  ...
  auto c = [&Himalaya, &Alps, &Karakoram, &Hindukush, &RockyMountains, &Ural,
            &Andes](const Eigen::Vector2d& x) -> double {
  ...
  for (int k = 1; k < 17; k++) {
    ...
    auto nodal_data_caps =
        lf::mesh::utils::make_CodimMeshDataSet<double>(mesh_p, 2);
    for (int global_idx = 0; global_idx < N_dofs; global_idx++) {
      nodal_data_caps->operator()(dofh.Entity(global_idx)) =
          car_cap.col(k - 1)[global_idx];
    }
