...
/* Function for the assembly of both Galerkin Matrices, the Mass matrix and the Stiffness matrix */ 
template <typename DIFF_COEFF, typename NONLOC_BC>
std::pair<Eigen::SparseMatrix<double>, Eigen::SparseMatrix<double>> 
	assembleGalerkinMatrices(const lf::assemble::DofHandler& dofh, DIFF_COEFF &&c, NONLOC_BC &&h, Eigen::MatrixXd L) {
	...
	lf::uscalfe::MassEdgeMatrixProvider<double, decltype(mf_h), decltype(edges_predicate)> elMat_Edge(fe_space, mf_h, edges_predicate);
...
StrangSplit::StrangSplit(const std::shared_ptr<lf::uscalfe::UniformScalarFESpace<double>> fe_space, double T, unsigned m, 
							double lambda, DIFF_COEFF &&c, NONLOC_BC &&h, Eigen::MatrixXd L) : fe_space_(fe_space), T_(T), m_(m), lambda_(lambda) {

	const lf::assemble::DofHandler &dofh{fe_space_->LocGlobMap()};	

	std::pair<Eigen::SparseMatrix<double>, Eigen::SparseMatrix<double>> galerkinpair = assembleGalerkinMatrices(dofh, c, h, L);
 ...
 */
Eigen::VectorXd StrangSplit::diffusionEvolutionOperator(double tau, const Eigen::VectorXd &mu) {
 ...
 */
Eigen::VectorXd StrangSplit::Evolution(Eigen::VectorXd &cap, const Eigen::VectorXd &mu) {
