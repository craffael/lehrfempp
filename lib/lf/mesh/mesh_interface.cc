...
namespace lf::mesh {
  ...
  void printInfo(const Mesh &mesh,std::ostream &o) {
    ...
    o << "Mesh of dimension " << dim_mesh
      << ", ambient dimension " << dim_world << std::endl;
    
    // Lopp over codimensions
    for (int co_dim = dim_mesh; co_dim >= 0; co_dim--) {
      const size_type no_ent = mesh.Size(co_dim);
      o << "Co-dimension " << co_dim << ": " << no_ent << " entities" << std::endl;
	...
	LF_VERIFY_MSG(e_geo_ptr,co_dim << "-entity " << e_idx
		      << ": missing geometry");
	...
	const Eigen::MatrixXd ref_el_corners(getRefElCorners(e_refel));
