...
MeshFactory::size_type MeshFactory::AddPoint(coord_t coord) {
  ...
  MeshFactory::size_type MeshFactory::AddEntity(
    ...
    LF_ASSERT_MSG(!built_, "Build() already called.");
    LF_ASSERT_MSG(ref_el.Dimension() > 0,"Use AddNode() to add a node to a mesh.");
    ...
    if (geometry != nullptr) {
      ...
      LF_ASSERT_MSG(geometry->RefEl() == ref_el,
	...
	LF_ASSERT_MSG(n < nodes_.size(),
		      "node " << n << " specified in call to AddEntity must be "
	...
	++count;
      ...
      LF_ASSERT_MSG(count == 2,"ref_el = segment but size of nodes was " << count);
...
std::shared_ptr<mesh::Mesh> MeshFactory::Build() {
  built_ = true;
  mesh::Mesh *mesh_ptr = new hybrid2dp::Mesh(dim_world_,nodes_,edges_,elements_);
