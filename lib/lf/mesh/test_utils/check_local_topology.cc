...
void checkLocalTopology(const Entity& e) {
  ...
  for (dim_t sub_codim = 1; sub_codim <= dimension; sub_codim++) {
    ...
    base::RandomAccessRange<const Entity> sub_ent_range(e.SubEntities(sub_codim));
	...
	base::RandomAccessRange<const Entity>
	  sub_sub_ent_range(sub_ent.SubEntities(sub_sub_codim));
	for (const Entity &sub_sub_ent : sub_sub_ent_range) {
	  ...
	  base::RandomAccessRange<const Entity>
	    e_sub_sub_range(e.SubEntities(sub_codim+sub_sub_codim));
