...
namespace lf::mesh::hybrid2d {
  ...
  using GeometryPtr = std::shared_ptr<geometry::Geometry>;
  
  using EdgeList = std::vector<std::pair<std::array<Mesh::size_type,2>,
					 GeometryPtr> >;
  using CellList = std::vector<std::pair<std::vector<Mesh::size_type>,
					 GeometryPtr> >;
  ...
  void mesh_from_node_incidence(std::vector<Eigen::VectorXd> nodes,
    ...
    */
    ...
    for (const auto &e : edges) {
      ...
      EdgeData edge_data { e.second , empty_cells_list };
      EdgeMap::value_type edge_info = std::make_pair(e_endpoint_idx,edge_data);
      std::pair<EdgeMap::iterator,bool> insert_status = edge_map.insert(edge_info);
      if (!insert_status.second) {
	std::cerr << "Duplicate edge " << end_nodes[0]
    ...
    for (const auto &c : cells) {
      ...
      if (no_of_vertices == 3) no_of_trilaterals++; else no_of_quadrilaterals++;
      
      // Visit all edges of the current cell
      for (int j=0;j<no_of_vertices;j++) {
	EndpointIndexPair c_edge_vertex_indices(cell_node_list[j],
						cell_node_list[(j+1)%no_of_vertices]);
	...
	if (edge_ptr == edge_map.end()) {
	  ...
	  EdgeData edge_data { edge_geo_ptr , single_cell_list };
	  std::pair<EdgeMap::iterator,bool> insert_status =
	    edge_map.insert(std::make_pair(c_edge_vertex_indices, edge_data));
    ...
    for (const EdgeMap::value_type &edge : edge_map) {
      ...
      GeometryPtr edge_geo_ptr(edge.second.first);
      /*
