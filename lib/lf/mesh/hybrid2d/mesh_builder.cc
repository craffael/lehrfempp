...
namespace lf::mesh::hybrid2d {
  ...
  std::unique_ptr<mesh::Mesh>  TPTriagMeshBuilder::Build() {
	...
	mesh->entities0_.emplace_back(mesh,node_cnt,point_ptr,sub_vertex);
      }
    // Initialize edges
	...
	mesh->entities1_.emplace_back(mesh,edge_cnt,seg_ptr,sub_edge);
      }
    // Next vertical edges
    for(int i=0;i<=nx;++i)
      for(int j=0;j<ny;++j,++edge_cnt) {
	...
	sub_edge[0] = std::vector<size_type>{first_endpoint_idx,second_endpoint_idx};
	// Coordinates of endpoints a columns of a 2x2 matrix
	Eigen::Matrix<double, Eigen::Dynamic, 2> edge_geo(2);
    ...
    const size_type diag_edge_offset = v_edge_offs + (nx+1)*ny;
	...
	// Construct geometry
	Eigen::Matrix<double, Eigen::Dynamic, 3> tria_geo_up(2);
	tria_geo_up << i*hx,(i+1)*hx,i*hx,j*hy,(j+1)*hy,(j+1)*hy;
	auto tria_geo_ptr = std::make_unique<geometry::TriaO1>(tria_geo);
	// Generate the triangle entity
	mesh->entities0_.emplace_back(mesh,tria_cnt,tria_geo_ptr,sub_tria);
	...
	// Construct geometry
	Eigen::Matrix<double, Eigen::Dynamic, 3> tria_geo_low(2);
	tria_geo_low << i*hx,(i+1)*hx,(i+1)*hx,j*hy,j*hy,(j+1)*hy;
	auto tria_geo_ptr = std::make_unique<geometry::TriaO1>(tria_geo);
	// Generate the triangle entity
	mesh->entities0_.emplace_back(mesh,tria_cnt+1,tria_geo_ptr,sub_tria);
      }
  }
