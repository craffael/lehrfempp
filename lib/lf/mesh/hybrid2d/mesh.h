...
public:
  ...
  Mesh(char dim_world, std::vector<Eigen::VectorXd> nodes,
       std::vector<std::tuple<std::vector<size_t>, std::unique_ptr<Geometry>>>
       elements);

  char DimMesh() const override {
    return 2;
  }
  ...
  size_t Size(char codim) const override {
    LF_ASSERT_MSG(codim >= 0, "codim negative.");
    LF_ASSERT_MSG(codim <= dim_world_, "codim > dimWorld.");
  ...
  size_t Index(const mesh::Entity& e) const override {
    switch (e.Codim()) {
      case 0:
