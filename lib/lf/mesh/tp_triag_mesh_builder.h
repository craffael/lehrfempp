...
namespace lf::mesh::hybrid2d {
  ...
  /**
   * @ brief Builder interface for creating structured meshes of rectangular domains
   *
  ...
  public:
     ...
     *
     */
    explicit StructuredMeshBuilder(std::shared_ptr<mesh::MeshFactory> mesh_factory)
  ...
  public:
    ...
    explicit TPTriagMeshBuilder(std::shared_ptr<mesh::MeshFactory> mesh_factory)
      ...
      LF_ASSERT_MSG(mesh_factory_->DimWorld() == 2,
  ...
  /**
   * @brief Implements a Builder for a tensor product grid (with rectangular cells)
   *
   * The generated grid will be uniform, which means that all cells are congruent.
   * The geoemtry of the rectangular domain and the number of cells can be specified
   * by setting Builder state parameters.
   * 
   */
