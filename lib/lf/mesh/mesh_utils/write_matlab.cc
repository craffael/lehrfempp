...
namespace lf::mesh::mesh_utils {
  ...
  bool writeMatlab(const lf::mesh::Mesh &mesh, std::string f_name) {
    ...
    if (file) {
      ...
      // Remove trailing .m
      f_name.pop_back(); f_name.pop_back();
      file << "function [x,y,TRI.EDS] = " << f_name << "()" << std::endl;

      // Obtain topological dimension of the mesh
      const dim_t dim_mesh = mesh.DimMesh();
      LF_VERIFY_MSG(dim_mesh == 2,"write_matlab() only avvailable for 2D meshes");
	...
	file << "x(" << node_cnt << ") = " << node_coord(0,0) << "; " << std::endl;
	file << "y(" << node_cnt << ") = " << node_coord(1,0) << "; " << std::endl;
	node_cnt++;
      }
	...
	file << "TRI(" << cell_cnt << ",:) = [" << mesh.Index(sub_ent[0])
	     << ", " << mesh.Index(sub_ent[1]) << ", "
	     << mesh.Index(sub_ent[2]) << "];" << std::endl;
	cell_cnt++;
      }
    }
  }
