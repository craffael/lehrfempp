...
class DynamicFEDofHandler : public DofHandler {
 public:
  ...
  for (glb_idx_t cell_idx=0; cell_idx < no_cells; cell_idx++) {
    ...
    for (int ed_sub_idx = 0; ed_sub_idx < cell->RefEl().NumSubEntities(1);
      ...
      const glb_idx_t edge_int_dof_offset = offsets_[1][edge_idx+1] - no_int_dof_edge;
  ...
  /** @copydoc DofHandler::getMesh()
   */
  virtual const lf::mesh::Mesh &getMesh(void) const override { return *mesh_p_; }
 ...
 private:
   ...
   *
   * The indices of global shape functions are stored in a long array `dofs_[codim]` for each 
   * co-dimension `codim`. The entries in the `offsets_[codim]` array point to the beginning
   * of the dof indices for a particular entity identified via its index in the mesh.
   *
   * @note the length `offsets_[codim]` vectors must exceed the number of corresponding
   *       entities by 1.
   */
  /**@{*/
