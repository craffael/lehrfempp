...
namespace lf::base {
  ...
  template <class T> int Track<T>::count = 0;

  template <class T>
  Track<T>::Track(std::string name,T &ref,Track<T>* &root,std::string comment)
    :name_(name),ref_(ref),prev_(nullptr),comment_(comment)
  {
    if (root) root->prev_ = this;
    next_ = root; root = this;
    count++;
  }
...
#define CONTROLDECLARE(intvar,varname) \
  static int intvar = 0;						\
  static lf::base::StaticVar ctrlvar##intvar(varname,intvar,lf::base::ctrl_root)

#define CLASSCONTROLDECLARE(class,intvar,varname)	\
  int class::intvar = 0;						\
  static lf::base::StaticVar class##intvar(varname,class::intvar,lf::base::ctrl_root)

#define CONTROLDECLARECOMMENT(class,intvar,varname,comment) \
  int class::intvar = 0;						\
  static lf::base::StaticVar class##intvar(varname,class::intvar,lf::base::ctrl_root,#comment)
/**@}*/
...
#define CTRLVARROOT
   ...
   *
   */
  unsigned int ListCtrlVars(std::ostream &o,
   ...
   */
  bool SetCtrlVar(std::string varname,int value,
		  const StaticVar *ctrl_var_root = nullptr);
   ...
   */
  bool ReadCtrlVarsFile(std::string filename,
			const StaticVar *ctrl_var_root = nullptr);
