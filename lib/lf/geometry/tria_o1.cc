...
TriaO1::TriaO1(Eigen::Matrix<double, Eigen::Dynamic, 3> coords)
  ...
  if (coords_.rows() == 2) {
    ...
    jacobian_inverse_gramian_ = jacobian_ * (jacobian_.transpose() * jacobian_).
                                inverse();
    integrationElement_ = std::
      sqrt((jacobian_.transpose() * jacobian_).determinant());
  }
}

Eigen::MatrixXd TriaO1::Global(const Eigen::MatrixXd& local) const {
  return coords_.col(0) * (1 - local.array().row(0) - local.array().row(1)).
         matrix()
