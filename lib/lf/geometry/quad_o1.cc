...
Eigen::MatrixXd QuadO1::Global(const Eigen::MatrixXd& local) const {
  return
    coords_.col(0) * ((1 - local.array().row(0)) * (1 - local.array().row(1))).
    matrix()
    + coords_.col(1) * (local.array().row(0) * (1 - local.array().col(1))).
    matrix()
    + coords_.col(2) * (local.array().row(0) * local.array().row(1)).matrix()
    + coords_.col(3) * ((1 - local.array().row(0)) * local.array().row(1)).
    matrix();
}
...
Eigen::MatrixXd QuadO1::Jacobian(const Eigen::MatrixXd& local) const {
  ...
  for (int i = 0; i < local.cols(); ++i) {
    ...
    result.col(2 * i + 1) = (coords_.col(3) - coords_.col(0)) *
                            (1 - local(0, i)) +
...
std::unique_ptr<Geometry> QuadO1::subGeometry(dim_t codim, dim_t i) const {
  ...
  switch (codim) {
    ...
    case 1:
      ...
      return
        make_unique<SegmentO1>((Eigen::Matrix<double, Eigen::Dynamic, 2>(DimGlobal(), 2)
                                << coords_.
                                col(RefEl().SubSubEntity2SubEntity(1, i, 1, 0)),
                                coords_.
                                col(RefEl().SubSubEntity2SubEntity(1, i, 1, 1)))
                               .finished());
    case 2:
      LF_ASSERT_MSG(i >= 0 && i < 4, "i is out of bounds.");
