...
std::ostream& operator<<(std::ostream& stream, const Geometry& geom) {
  ...
  double Volume(const Geometry &geo) {
    ...
    double refvol = 1.0;
    switch (geo.RefEl()) {
    case lf::base::RefEl::kPoint(): {  return 1.0;  }
    case lf::base::RefEl::kSegment():  { refc << 0.5; break; }
    case lf::base::RefEl::kTria(): { refvol = 0.5; refc << 1.0/3,1.0/3; break; }
    case lf::base::RefEl::kQuad(): { refc << 0.5,0.5; break; }
    default: {
      LF_VERIFY_MSG(false,"Volume not available for " << geo.RefEl().ToString());
    }
    } // end switch RefEl
