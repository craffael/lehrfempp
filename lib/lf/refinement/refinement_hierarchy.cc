...
namespace lf::refinement {
    ...
    mesh_factory_(mesh_factory)
  {
    ...
    meshes_.push_back(base_mesh);
    {
      ...
      refinement_edges_.push_back(std::vector<lf::base::sub_idx_t>(base_mesh->Size(0),-1));
	...
	if (cell.RefEl() == lf::base::RefEl::kTria()) {
      ...
      child_infos_.push_back({
	  std::move(cell_child_info),std::move(edge_child_info),std::move(point_child_info) });
    }
    {
	  ...
	  std::move(cell_parent_info),std::move(edge_parent_info),std::move(point_parent_info)});
    }
  }
  ...
  template <typename Marker> void MeshHierarchy::MarkEdges(Marker &&marker) {
    // Retrieve the finest mesh in the hierarchy
    const mesh::Mesh &finest_mesh(*meshes_.back());
  ...
  void MeshHierarchy::PerformRefinement(void) {
	...
	case RefPat::rp_copy: {
	  ...
	  std::vector<std::unique_ptr<lf::geometry::Geometry>>
	    ed_copy(edge.Geometry()->ChildGeometry(rp_copy,0));
	  LF_VERIFY_MSG(ed_copy.size() == 1,
			"Copy creates only a single child!");
