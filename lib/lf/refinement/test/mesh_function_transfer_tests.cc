...
TEST(lf_refinement, MeshFunctionTransferScalar) {
    ...
    functions.push_back([](const Eigen::Vector2d& x) { return std::sin(x[0]+x[1]); });
	...
	for (const auto& f : functions) {
		...
		ASSERT_NEAR(value_global, value_transferred, 1e-10) << "value_global=" << value_global << "\tvalue_transferred=" << value_transferred;
	    }
	}
    }
}
