...
void check_lagr_interp_nodes(std::shared_ptr<lf::mesh::Mesh> mesh_coarse, std::shared_ptr<const FES_COARSE> fes_coarse, const Eigen::VectorXd& dofs_coarse, lf::refinement::RefPat ref_pat) {
    ...
    const auto dofs_fine = lf::intergridfe::prolongate(mh, fes_coarse, fes_fine, dofs_coarse, 0);
...
TEST(lf_intergridfe, LagrInterpNodes) {
	...
	const auto fes_coarse_o1 = std::make_shared<lf::uscalfe::FeSpaceLagrangeO1<double>>(mesh_coarse);
	const auto fes_coarse_o2 = std::make_shared<lf::uscalfe::FeSpaceLagrangeO2<double>>(mesh_coarse);
	// Test for all possible refinement patterns
	for (const auto ref_pat : {lf::refinement::rp_regular,
				   lf::refinement::rp_barycentric}) {
		...
		dofs[dof] = 1;
		check_lagr_interp_nodes<lf::uscalfe::FeSpaceLagrangeO1<double>, lf::uscalfe::FeSpaceLagrangeO1<double>>(mesh_coarse, fes_coarse_o1, dofs, ref_pat);
		check_lagr_interp_nodes<lf::uscalfe::FeSpaceLagrangeO1<double>, lf::uscalfe::FeSpaceLagrangeO2<double>>(mesh_coarse, fes_coarse_o1, dofs, ref_pat);
	    }
		...
		dofs[dof] = 1;
		check_lagr_interp_nodes<lf::uscalfe::FeSpaceLagrangeO2<double>, lf::uscalfe::FeSpaceLagrangeO1<double>>(mesh_coarse, fes_coarse_o2, dofs, ref_pat);
		check_lagr_interp_nodes<lf::uscalfe::FeSpaceLagrangeO2<double>, lf::uscalfe::FeSpaceLagrangeO2<double>>(mesh_coarse, fes_coarse_o2, dofs, ref_pat);
	    }
	}
    }
}
