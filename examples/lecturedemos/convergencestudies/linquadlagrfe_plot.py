...
L2_p1_meshwidth = np.polyfit(np.log(meshwidth), np.log(L2_err_o1), 1)
L2_p2_meshwidth = np.polyfit(np.log(meshwidth), np.log(L2_err_o2), 1)
L2_p1_ndofs = np.polyfit(np.log(ndofs_o1), np.log(L2_err_o1), 1)
L2_p2_ndofs = np.polyfit(np.log(ndofs_o2), np.log(L2_err_o2), 1)
...
plt.loglog(meshwidth, np.exp(L2_p1_meshwidth[0]*np.log(meshwidth) + L2_p1_meshwidth[1]), '--', label='slope: '+str(round(L2_p1_meshwidth[0], 2)))
plt.loglog(meshwidth, np.exp(L2_p2_meshwidth[0]*np.log(meshwidth) + L2_p2_meshwidth[1]), '--', label='slope: '+str(round(L2_p2_meshwidth[0], 2)))
plt.loglog(meshwidth, L2_err_o1, 'o', label='Linear FE')
plt.loglog(meshwidth, L2_err_o2, 'o', label='Quadratic FE')
plt.legend(loc='lower left')
plt.grid()
plt.xlabel('Mesh width [log]')
plt.ylabel('Discretization error [log]')
plt.title('Discretization errors with respect to $L^2$ norm')
plt.gca().invert_xaxis()
plt.savefig(os.path.join(output_folder, 'Q_L2_meshwidth.eps'))

plt.gcf().clear()

plt.loglog(ndofs_o1, np.exp(L2_p1_ndofs[0]*np.log(ndofs_o1) + L2_p1_ndofs[1]), '--', label='slope: '+str(round(L2_p1_ndofs[0], 2)))
plt.loglog(ndofs_o2, np.exp(L2_p2_ndofs[0]*np.log(ndofs_o2) + L2_p2_ndofs[1]), '--', label='slope: '+str(round(L2_p2_ndofs[0], 2)))
plt.loglog(ndofs_o1, L2_err_o1, 'o', label='Linear FE')
plt.loglog(ndofs_o2, L2_err_o2, 'o', label='Quadratic FE')
plt.legend(loc='lower left')
plt.grid()
plt.xlabel('DOFs [log]')
plt.ylabel('Discretization error [log]')
plt.title('Discretization errors with respect to $L^2$ norm')
plt.savefig(os.path.join(output_folder, 'Q_L2_dofs.eps'))
