...
int main(int argc, char *argv[]) {
    ...
    const auto [quad_points, quad_weights] = lf::quad::GaussLegendre(num_quad_points);

    // Load function
    const auto f = [](double x) {
	return -4*M_PI * (std::cos(2*M_PI*x*x) - 4*M_PI*x*x*std::sin(2*M_PI*x*x));
    };
    // Analytic solution
    const auto u = [](double x) {
	...
	// Generate the stiffness matrix for an equidistant mesh on [0, 1] with M cells
	// and first order lagrangian finite elements
	...
	for (int i = 0 ; i < M ; ++i) {
	    ...
	    const Eigen::VectorXd loc_quad_points = Eigen::VectorXd::Constant(num_quad_points, a) + h*quad_points;
		...
		rhs[i]   += loc_quad_weights[k] * b2(loc_quad_points[k]) * f(loc_quad_points[k]);
		rhs[i+1] += loc_quad_weights[k] * b1(loc_quad_points[k]) * f(loc_quad_points[k]);
	    }
	}
	...
	const Eigen::VectorXd cell_values_analytic = Eigen::VectorXd::LinSpaced(M, h/2, 1.-h/2).unaryExpr(u);
	const Eigen::VectorXd cell_values_approximate = (sol.head(M) + sol.tail(M)) / 2;
	double norm_max = (cell_values_approximate - cell_values_analytic).array().abs().maxCoeff();
	...
	for (int i = 0 ; i < M ; ++i) {
	    ...
	    // The difference in the gradient of the approximate and the exact solution
	    ...
	    for (int k = 0 ; k < num_quad_points ; ++k) {
		norm_H1_squared += loc_quad_weights[k] * diff_grad(loc_quad_points[k])*diff_grad(loc_quad_points[k]);
		norm_L2_squared += loc_quad_weights[k] * diff(loc_quad_points[k])*diff(loc_quad_points[k]);
	    }
	}
	results((M/dM)-1, 0) = M;
