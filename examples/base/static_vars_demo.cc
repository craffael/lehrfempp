...
CONTROLDECLARECOMMENT(StaticVarsDemoClass, other_var_, "other_var", "A test case");

CONTROLDECLARECOMMENT(StaticVarsDemoClass, arg_var_, "arg_var","Set from command line");

CONTROLDECLARECOMMENT(StaticVarsDemoClass, output_ctrl_, "output_ctrl_", "Test output ctrl");

int main(int argc,const char *argv[]) {
  std::cout << "LehrFEM++ Demo program for the use of static control variables" << std::endl;
  std::cout << "Variables can be set via command line arguments: VARNAME=VALUE" << std::endl;
  std::cout << "Try: testvar=42" << std::endl << std::endl;
				    
  std::cout << ">> List of control variables before initialization:" << std::endl;
  ...
  std::cout << ">> List of control variables after initialization:" << std::endl;
  ListCtrlVars(std::cout);
